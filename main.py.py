...
def load_image_into_numpy_array(image_bytes):
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to process the image. Error: {e}")

def numpy_to_image_bytes(image_np):
    success, buffer = cv2.imencode('.png', image_np)
    if not success:
        raise HTTPException(status_code=500, detail="Failed to encode the image.")
    return BytesIO(buffer.tobytes())

@app.post("/process_image_enhancement_from_image_file")
async def process_image_enhancement_from_image_file(file: UploadFile = File(...)):
    try:
        image_data = await file.read()
        image_np = load_image_into_numpy_array(image_data)
        image_processor = ImageProcessingOCR(image_np)
        enhanced_image = image_processor.process_image_enhancement()
        enhanced_image_bytes_io = numpy_to_image_bytes(enhanced_image)
        return StreamingResponse(enhanced_image_bytes_io, media_type="image/png")
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Error processing the image: {e}")

@app.post("/process_image_enhancement_from_image_url")
async def process_image_enhancement_from_image_url(image_url: str):
    try:
        response = requests.get(image_url)
        if response.status_code != 200:
            raise HTTPException(status_code=400, detail="Unable to download image from URL")
        image_np = load_image_into_numpy_array(response.content)
        image_processor = ImageProcessingOCR(image_np)
        enhanced_image = image_processor.process_image_enhancement()
        enhanced_image_bytes_io = numpy_to_image_bytes(enhanced_image)
        return StreamingResponse(enhanced_image_bytes_io, media_type="image/png")
